...
type FilterConfig struct {
	...
	URLReverse    bool `json:"url_reverse,omitempty"`
	...
	URLFilterMode    int `json:"url_filter_mode,omitempty"`
	...
	// 导出过滤逻辑：0=与(默认，所有条件都需满足), 1=或(满足任一条件即可)
	ExportFilterLogic int `json:"export_filter_logic,omitempty"`
}
...
func MergeConfig(configFile *FilterConfig, cmdFields string, cmdTopN int, cmdSortBy string, cmdCsvTop int, cmdWorkers int, cmdBatchSize int, cmdOutput string, cmdLogPath string, cmdStartTime string, cmdEndTime string, cmdSIPFilters []string, cmdDIPFilters []string, cmdDomainFilters []string, cmdSportFilters []string, cmdDportFilters []string, cmdURLFilters []string, cmdSIPReverse bool, cmdDIPReverse bool, cmdDomainReverse bool, cmdSportReverse bool, cmdDportReverse bool, cmdURLReverse bool, cmdSIPFilterMode int, cmdDIPFilterMode int, cmdDomainFilterMode int, cmdSportFilterMode int, cmdDportFilterMode int, cmdURLFilterMode int, cmdPprofSwitch bool, cmdFlushThreshold int, cmdExportFilterLogic int) (*FilterConfig, error) {
	...
	if configFile == nil {
		return &FilterConfig{
			...
			ExportFilterLogic: cmdExportFilterLogic,
		}, nil
	}
	...
	merged := &FilterConfig{
		...
		URLFilters:        configFile.URLFilters,
		...
		URLReverse:        configFile.URLReverse,
		...
		ExportFilterLogic: configFile.ExportFilterLogic,
	}
	...
	if len(cmdURLFilters) > 0 {
		merged.URLFilters = cmdURLFilters
	}
	...
	if cmdURLReverse {
		merged.URLReverse = cmdURLReverse
	}
	...
	// 合并导出过滤逻辑（命令行参数覆盖配置文件参数）
	if cmdExportFilterLogic != 0 {
		merged.ExportFilterLogic = cmdExportFilterLogic
	}
